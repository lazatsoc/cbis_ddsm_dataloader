...
class CenteredPatches(torch.nn.Module):
    def __init__(self, patch_size):
        super(CenteredPatches, self).__init__()
    ...
    def forward(self, sample):
        ...
        minx_naive = int(cx - self.patch_size[0] / 2)
        minx = max((0, minx_naive))
        dx1 = minx - minx_naive

        maxx_naive = int(cx + self.patch_size[0] / 2)
        ...
        miny_naive = int(cy - self.patch_size[1] / 2)
        miny = max((0, miny_naive))
        dy1 = miny - miny_naive

        maxy_naive = int(cy + self.patch_size[1] / 2)
    ...
    def __repr__(self):
        detail = f"(patch_size={self.patch_size})"
